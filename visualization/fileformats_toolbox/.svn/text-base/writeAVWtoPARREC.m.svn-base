function outpar=writeAVWtoPARREC(analyzeFileTag,parrecFile,parrecHDR)
...
if(~caps)
    fp =fopen([parrecFile '.rec'],'wb','ieee-le');
else
    fp =fopen([parrecFile '.REC'],'wb','ieee-le');
end
for j=1:length(nFiles)
    avw = avw_img_read([analyzeFileTag '_Vol_' sprintf('%03d',j)]);
        ...
        case 'SAG'
            ...
            error('writeAVWtoPARREC: Orient SAG not supported yet. Contact bennett@bme.jhu.edu with example data and we will correct this problem.')
        case 'COR'
            % Coronal                        
            order = [1 3 2];
    ...
    for k=1:size(avw.img,3)
        slice = avw.img(:,:,k)*hdr.img(1).vis.scale_slope; % default scaling set by PAR header
        fwrite(fp,slice(:),'uint16');
    end
end
fclose(fp);
