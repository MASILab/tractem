...
%
%	7. userhos: If specified and userhos=1, the program will use 
%		Right Hand Oriented System (RHOS) to display; otherwise,
%		it will use the standard Analyze orientation to display.
%		For 'update' command, the program will check the current
%		status.userhos. If it is different from option.userhos,
%		the program will properly rotate the display.
%
%	8. useinterp: If specified and useinterp=1, the image will be
...
%
%
%	9. setarea: 3-View plot will be displayed on this specific 
%		region. If it is not specified, program will set the
%		plot area to [0.05 0.05 0.9 0.9].
%
%	10. setunit: It can be specified to setunit='voxel' or 'mm'
%		and the view will change the axes unit of [X Y Z]
%		accordingly.
%
%	11. setviewpoint: If specified, [X Y Z] values will be used
%		to set the viewpoint of 3-View plot.
%
%	12. setscanid: If specified, [t] value will be used to display
%		the specified image scan in NIFTI data.
%
%	13. setcrosshaircolor: If specified, [r g b] value will be used
%		for Crosshair Color. Otherwise, red will be the default.
%
%	14. setcolorindex: If specified, the 3-View will choose the
...
%
%	15. setcolormap: 3-View plot will use it as a customized colormap.
...
%
%	16. setcolorlevel: If specified (must be no more than 256, and
...
%
%	17. sethighcolor: If specified, program will squeeze down the
...
%
%	18. setcbarminmax: if specified, the [min max] will be used to
%		set the min and max of the colorbar, which does not
%		include any data for highcolor.
%
%	19. setvalue: If specified, setvalue.val (with the same size as
...
%
%	20. glblocminmax: If specified, pgm will use glblocminmax to
%		calculate the colormap, instead of minmax of image.
%
%	21. setbuttondown: If specified, pgm will evaluate the command
%		after a click or slide action is invoked to the new
%		view point.
%
%	22. setcomplex: This option will decide how complex data to be
...
function status = view_nii(varargin)
   ...
   case {'init'}
      ...
      fig = init(nii, fig, setarea, setunit, setviewpoint, setscanid, setbuttondown, ...
         ...
         usecolorbar, usepanel, usecrosshair, usestretch, useimagesc, userhos, ...
         useinterp, setvalue, glblocminmax, setcrosshaircolor, setcomplex);
   ...
   case {'axial_image','coronal_image','sagittal_image'}    
      ...
      if sag,
         ...
         if nii_view.userhos
            nii_view.slices.sag = nii_view.dims(1) - nii_view.slices.sag + 1;
         end
      end;
   ...
   case {'axial_slider','coronal_slider','sagittal_slider'},
      ...
      if sag,
         if nii_view.userhos
            if isfield(nii_view.handles,'sagittal_bg') & ~isempty(nii_view.handles.sagittal_bg) & nii_view.useinterp
               Ssag = squeeze(nii_view.bgimg(nii_view.dims(1)-nii_view.slices.sag+1,:,:));
               set(nii_view.handles.sagittal_bg,'CData',Ssag');
            end

            if isfield(nii_view.handles,'sagittal_image'),
               Ssag = squeeze(img(nii_view.dims(1)-nii_view.slices.sag+1,:,:));
               set(nii_view.handles.sagittal_image,'CData',Ssag');
            end

            if isfield(nii_view.handles,'sagittal_slider'),
               set(nii_view.handles.sagittal_slider,'value', ...
			nii_view.dims(1) - nii_view.slices.sag + 1);
            end
         else
            if isfield(nii_view.handles,'sagittal_bg') & ~isempty(nii_view.handles.sagittal_bg) & nii_view.useinterp
               Ssag = squeeze(nii_view.bgimg(nii_view.slices.sag,:,:));
   ...
   case {'impos_edit'}
      ...
      if nii_view.userhos
         impos(1) = nii_view.dims(1) - nii_view.slices.sag + 1;
      else
...
function fig = init(nii, fig, area, setunit, setviewpoint, setscanid, buttondown, ...
         ...
         userhos, useinterp, setvalue, glblocminmax, setcrosshaircolor, ...
         setcomplex)
   ...
   if userhos
      ...
      nii.hdr.dime.pixdim([2 3]) = nii.hdr.dime.pixdim([3 2]);
      nii.hdr.hist.originator([1 2]) = nii.hdr.hist.originator([2 1]);
      nii.img=flipdim(permute(nii.img,[2 1 3]), 1);

      if ~isempty(bgimg)
         bgimg=flipdim(permute(bgimg,[2 1 3]), 1);
      end
   end
   ...
   if xdim > 1,
      ...
      if userhos
         set(handles.sagittal_slider,'value',xdim-nii_view.slices.sag+1);
      end
   end
...
%      set(handles.Thist, 'visible', 'on');
   ...
   if ~isempty(nii_view.bgimg)
      if userhos
         bg_slice = squeeze(bgimg(xdim-nii_view.slices.sag+1,:,:));
      else
   ...
   if userhos
      img_slice = squeeze(nii.img(xdim-nii_view.slices.sag+1,:,:));
   else
      if nii.hdr.dime.datatype == 128 | nii.hdr.dime.datatype == 511
   ...
   [top1_label, top2_label, side1_label, side2_label] = ...
	dir_label(fig, top_ax, front_ax, side_ax, userhos);
...
if 0
   ...
   nii_view.userhos = userhos;		% right handed orientation system
...
function fig = update_img(img, fig, opt)
   ...
   if isfield(opt, 'setvalue')
      ...
      if nii_view.userhos
         img = flipdim(permute(img,[2 1 3]), 1);
      end
   ...
   if nii_view.userhos
      ...
      if ~isempty(nii_view.bgimg)
         nii_view.bgimg=flipdim(permute(nii_view.bgimg,[2 1 3]), 1);
      end
   end
   ...
   if nii_view.userhos
      if ~isempty(nii_view.bgimg)
         Ssag = squeeze(nii_view.bgimg(size(nii_view.nii.img,1)-nii_view.slices.sag+1,:,:));
      ...
      if isfield(nii_view.handles,'sagittal_image'),
         Ssag = squeeze(nii_view.nii.img(size(nii_view.nii.img,1)-nii_view.slices.sag+1,:,:));
         set(nii_view.handles.sagittal_image,'CData',Ssag');
      end
   else
      if ~isempty(nii_view.bgimg)
...
function set_image_value(nii_view),
    ...
    if nii_view.userhos
       sag = nii_view.dims(1) - round(nii_view.slices.sag) + 1;
    else
...
function nii_view = update_nii_view(nii_view)
   ...
   if nii_view.userhos
      xlim = get(nii_view.handles.axial_image, 'xdata');
      p_axi(1) = xlim(end) - (p_axi(1) - xlim(1));
      p_cor(1) = xlim(end) - (p_cor(1) - xlim(1));
   end
...
function hist_eq(fig)
   ...
   %
   if nii_view.userhos
      img_slice = squeeze(double(nii_view.disp(size(nii_view.disp,1)-nii_view.slices.sag+1,:,:)));
   else
...
function update_viewpoint(fig, setviewpoint)
   ...
   if 0 % nii_view.userhos
      setviewpoint = setviewpoint([2 1 3]);
   end
...
%----------------------------------------------------------------
function nii = rhos_convert(fig, nii, isanalyze2rhos)

   if ~isanalyze2rhos		% userhos (isanalyze2rhos) = 0
      nii.hdr.dime.dim([2 3]) = nii.hdr.dime.dim([3 2]);
      nii.hdr.dime.pixdim([2 3]) = nii.hdr.dime.pixdim([3 2]);
      nii.hdr.hist.originator([1 2]) = nii.hdr.hist.originator([2 1]);
      nii.img=flipdim(permute(nii.img,[2 1 3]), 2);
   end

   s = view_nii(fig);
   opt.usecolorbar = s.usecolorbar;
   opt.usepanel = s.usepanel;
   opt.usecrosshair = s.usecrosshair;
   opt.usestretch = s.usestretch;
   opt.useimagesc = s.useimagesc;
   opt.userhos = isanalyze2rhos;
   opt.setarea = s.area;
   opt.setunit = s.unit;
%   opt.setviewpoint = s.viewpoint([2 1 3]);
   opt.setcolorindex = s.colorindex;
   opt.setcolormap = s.colormap;
   opt.setcolorlevel = s.colorlevel;
   opt.sethighcolor = s.highcolor;
   opt.setbuttondown = s.buttondown;
   opt.command = 'updatenii';			% because of rhos
   view_nii(fig, nii, opt);
...
%----------------------------------------------------------------
function update_rhos(fig, userhos)
   ...
   nii_view = getappdata(fig,'nii_view');
   isrhos = nii_view.userhos;

   %  status not change
   %
   if (~userhos & ~isrhos) | (userhos & isrhos)
      return;
   end

   rhos_convert(fig, nii_view.nii, userhos);
...
function status = get_status(h);
   ...
   status.userhos = nii_view.userhos;
   ...
   if nii_view.userhos
      status.viewpoint = status.viewpoint([2 1 3]);
   end
...
function move_cursor(fig)
   ...
   elseif	curr(1) >= sag(1) & curr(1) <= sag(1)+sag(3) & ...
      ...
      if nii_view.userhos
         sag = nii_view.dims(1) - nii_view.slices.sag + 1;
      else
    ...
    if nii_view.userhos
       sag = nii_view.dims(1) - round(sag) + 1;
    else
       sag = round(sag);
    end
